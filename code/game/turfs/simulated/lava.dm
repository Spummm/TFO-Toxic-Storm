...
/turf/open/acidpus
	name = "acidic pus"
	icon = 'icons/turf/acid.dmi'
	icon_state = "unsmooth"
	desc = "A volatile sea of acidic, septic blood. Going in this would be a very, very bad idea. It seems to glow slightly."
	gender = PLURAL //"That's some acidic pus."
	baseturfs = /turf/open/acidpus //acid the whole way down
	slowdown = 2
...
/turf/open/acidpus/ex_act(severity, target)
	contents_explosion(severity, target)

/turf/open/acidpus/MakeSlippery(wet_setting, min_wet_time, wet_time_to_add, max_wet_time, permanent)
	return

/turf/open/acidpus/Melt()
	to_be_destroyed = FALSE
	return src

/turf/open/acidpus/acid_act(acidpwr, acid_volume)
	return

/turf/open/acidpus/MakeDry(wet_setting = TURF_WET_WATER)
	return

/turf/open/acidpus/airless
	initial_gas_mix = AIRLESS_ATMOS

/turf/open/acidpus/Entered(atom/movable/AM)
	if(burn_stuff(AM))
		START_PROCESSING(SSobj, src)

/turf/open/acidpus/hitby(atom/movable/AM, skipcatch, hitpush, blocked, datum/thrownthing/throwingdatum)
	if(burn_stuff(AM))
		START_PROCESSING(SSobj, src)

/turf/open/acidpus/process()
	if(!burn_stuff())
		STOP_PROCESSING(SSobj, src)

/turf/open/acidpus/rcd_vals(mob/user, obj/item/construction/rcd/the_rcd)
	switch(the_rcd.mode)
		if(RCD_FLOORWALL)
			return list("mode" = RCD_FLOORWALL, "delay" = 0, "cost" = 3)
	return FALSE

/turf/open/acidpus/rcd_act(mob/user, obj/item/construction/rcd/the_rcd, passed_mode)
	switch(passed_mode)
		if(RCD_FLOORWALL)
			to_chat(user, "<span class='notice'>You build a floor.</span>")
			PlaceOnTop(/turf/open/floor/plating, flags = CHANGETURF_INHERIT_AIR)
...
/turf/open/acidpus/singularity_act()
	return

/turf/open/acidpus/singularity_pull(S, current_size)
	return

/turf/open/acidpus/get_smooth_underlay_icon(mutable_appearance/underlay_appearance, turf/asking_turf, adjacency_dir)
	underlay_appearance.icon = 'icons/turf/floors.dmi'
	underlay_appearance.icon_state = "basalt"
	return TRUE
...
/turf/open/acidpus/TakeTemperature(temp)

/turf/open/acidpus/attackby(obj/item/C, mob/user, params)
	..()
	if(istype(C, /obj/item/stack/rods/lava))
		var/obj/item/stack/rods/lava/R = C
		var/obj/structure/lattice/lava/H = locate(/obj/structure/lattice/lava, src)
...
/turf/open/acidpus/proc/is_safe()
	//if anything matching this typecache is found in the lava, we don't burn things
	var/static/list/lava_safeties_typecache = typecacheof(list(/obj/structure/lattice/catwalk, /obj/structure/stone_tile, /obj/structure/lattice/lava))
	var/list/found_safeties = typecache_filter_list(contents, lava_safeties_typecache)
	for(var/obj/structure/stone_tile/S in found_safeties)
...
/turf/open/acidpus/proc/burn_stuff(AM)
	. = 0
	...
	for(var/thing in thing_to_check)
		if(isobj(thing))
			...
			if(!(O.resistance_flags & ACID_PROOF))
				O.resistance_flags |= ACID_PROOF //Even acidproof things burn up in lava
			if(O.resistance_flags & ACID_PROOF)
...
/turf/open/acidpus/smooth
	name = "acidic pus"
	baseturfs = /turf/open/acidpus/smooth
	icon = 'icons/turf/acid.dmi'
	icon_state = "unsmooth"
	smooth = SMOOTH_MORE | SMOOTH_BORDER
	canSmoothWith = list(/turf/open/acidpus/smooth)

/turf/open/acidpus/smooth/planet_surface
	initial_gas_mix = LAVALAND_DEFAULT_ATMOS
	planetary_atmos = TRUE
	baseturfs = /turf/open/acidpus/smooth/planet_surface

/turf/open/acidpus/smooth/airless
	initial_gas_mix = AIRLESS_ATMOS